...
class MyWindow(object):
    ...
    '''
    Call string parameters vch and hch cannot be defaulted to curses.ACS_VLINE and curses.ACS_HLINE,
    respectively, in the call string because initscr() has not yet been called when the call string is
    compiled, before any code has run. They can be referenced in the method body because that is
    executed during run time, after initscr() has been called.
    '''
    ...
    def draw_border(self):
        ...
        if self.windowType != 'popup' and self.windowType != 'message':
            self.win.addstr(0, self.width-10, 'Page %d/%d'% (self.currpage, self.pages),
                            curses.A_STANDOUT)
    ...
    def draw_contents(self, lastpage=False):
        ...
        for i in range(firstrow, lastrow):
            try:
                ...
                quit('Exception in drawContents trying to add row i=', i, ' "'+self.contents[i]+'" at '
                     '(%d,%d) page=%d, firstrow=%d, lastrow=%d'
                     %(myx, myy, self.currpage, firstrow, lastrow))
