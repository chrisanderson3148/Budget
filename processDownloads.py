...
def clearCUChecks():
    ...
    for inner_row in cur1:
        ...
        updated += 1
        # print 'Updated check '+checknum+' clear_date in checks table to '+str(trandate)
    db.commit()
...
def printUnknownNonCheckTransactions():
    ...
    try:
        query = 'select tran_date,tran_desc,tran_amount from main where bud_category = "UNKNOWN" and tran_date > "2005-12-31" and tran_desc not like "CHECK%" and tran_desc not like "Check%" order by tran_date;'
        cur1.execute(query)
    except:
        (etype,value,tb) = sys.exc_info()
    ...
    amount = 0
    for inner_row in cur1:
        print '%-12s %-40s $%10.2f' % (inner_row[0].strftime('%m/%d/%Y'), inner_row[1][:40], inner_row[2])
        amount += inner_row[2]
    print '-----------------------------------------------------------------'
...
def printUnRecordedChecks():
    ...
    try:
        query = 'select tran_checknum,tran_date,tran_amount from main where tran_checknum != "0" and tran_type = "b";'
        cur1.execute(query)
    except:
        (etype,value,tb) = sys.exc_info()
    ...
    for inner_row in cur1: # for every cleared CU check, see if it also exists as a transaction in the checks database, with at least an amount
        try:
            query = 'select tchecknum from checks where tchecknum = "'+str(inner_row[0])+'" and tamt is not null;'
            cur2.execute(query)
        except:
            (etype,value,tb) = sys.exc_info()
            print 'printUnRecordedChecks(): Exception executing query: '+query
        ...
        if cur2.rowcount == 0:
            print '%-5d %-12s $%7.2f' % (inner_row[0], inner_row[1].strftime('%m/%d/%Y'), abs(inner_row[2]))
...
def printUnclearedChecks():
    ...
    try:
        query = 'select tnum,tdate,tamt,tpayee,comments from checks where clear_date is null and tdate > "2005-12-31" order by tnum;'
        cur1.execute(query)
    except:
        (etype,value,tb) = sys.exc_info()
    ...
    for inner_row in cur1:
        ...
        mydict[key] = '%-5s %10s $%7.2f %-30s %s' % (inner_row[0], inner_row[1].strftime('%m/%d/%Y'), abs(inner_row[2]), inner_row[3], inner_row[4])

    try:
        query = 'select tnum,tdate,tamt,tpayee,comments from chasechecks where clear_date is null and tdate > "2005-12-31" order by tnum;'
        cur1.execute(query)
    except:
        (etype,value,tb) = sys.exc_info()
        print 'printUnclearedChecks(): Exception executing query: '+query
...
def insertDictIntoChecksDB(downloadDict, keysdict):
    ...
    for key, val in downloadDict.iteritems():
        if key in keysdict or key+'-0' in keysdict: continue # existing record found -- ignore
        ...
        query = 'insert into checks (tnum,tchecknum,tamt,tdate,tpayee,bud_cat,bud_amt,bud_date,comments) values ("'+val[0]+'","'+val[0]+'","'+val[1]+'","STR_TO_DATE("'+val[2]+'","%m/%d/%Y"),"'+val[3]+'","'+val[4]+'","'+val[5]+'",STR_TO_DATE("'+val[6]+'","%m/%d/%Y"),"'+val[7]+'");'
        ...
        print 'Key '+key+' is not in "checks" database -- '+('' if doinsert else 'would have ')+'inserted'
        inserted += 1
...
def insertDictIntoMainDB(downloadDict, keysdict):
    ...
    for key, val in downloadDict.iteritems():
        if '|' in key:
            ...
            oldkey = key
            newkey = key
        if oldkey in keysdict or oldkey+'-0' in keysdict or newkey in keysdict or newkey+'-0' in keysdict: # don't insert existing records into database, but check if should UPDATE the existing record
            #print oldkey+' found in database - not inserting'
            continue

        query = 'INSERT into main (tran_date,tran_ID,tran_desc,tran_checknum,tran_type,tran_amount,bud_category,bud_amount,bud_date,comment) VALUES (STR_TO_DATE("'+val[0]+'","%m/%d/%Y"), "'+newkey+'", "'+val[2]+'", "'+(val[3] if val[3] else "0") +'", "'+val[4]+'", "'+val[5]+'", "'+val[6]+'", "'+str(val[7])+'", STR_TO_DATE("'+val[8]+'","%m/%d/%Y"), "'+val[9]+'");'
        ...
        if doinsert:
            ...
            val[1] = newkey
            print 'Key '+newkey+' is not in "main" database -- '+('' if doinsert else 'would have ')+'inserted', val
        inserted += 1
...
if os.path.isfile(difile):
    ...
    DC_dict = tf.readMonthlyDiscoverFile(difile, True) # process a download file, not a monthly file
...
print '\n'+('Inserted' if doinsert else 'Did not insert'), inserted, 'records into DB'
