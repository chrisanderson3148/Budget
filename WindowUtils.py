...
def popup_message_ok(s, message, sw, log, title=' Message '):
    ...
    if type(message) is str:
        ...
        if len(message) > _max_len:
            ...
            _my_win.create(_my_ht, _max_len+5, top=max(1, _my_win.sheight/3 - _my_ht/2), left=max(1, (_my_win.swidth-(_max_len+5))/2), title=title)
            WindowList.add_window(_my_win.win)
            _my_win.win.bkgd(' ', curses.color_pair(5))
            y = 2
            ...
            _my_win.create(5, _my_wid, top=max(1, (_my_win.sheight/3 - 2)), left=max(1, (_my_win.swidth-_my_wid)/2), title=title)
            WindowList.add_window(_my_win.win)
            _my_win.win.bkgd(' ', curses.color_pair(5))
            _my_win.win.addstr(2, 2, message)
        ...
        _my_win.create(4+len(message), _wid, top=max(1, (_my_win.sheight/3 - (4+len(message))/2)), left=max(1, (_my_win.swidth-_wid)/2), title=title)
        WindowList.add_window(_my_win.win)
        _my_win.win.bkgd(' ', curses.color_pair(5))
        y = 2
        for _msg in message:
...
'''
Displays a short list of strings horizontally that the user can choose from, tabbing to highlight and select.
e.g., choice = popupGetMultipleChoice('Choose one:', ['Yes', 'No', 'Elephant', 'Mouse', 'Green', 'Red'], 'Mouse')
'''
def popup_get_multiple_choice(s, wintitle, choices, default, sw, log):
    ...
    if len(wintitle) > _len_choices:
        ...
        _win_wid = _len_choices + 5 # add some buffer on each end of the list of choices

    _my_win.create(5, _win_wid, top=max(1, (_my_win.sheight/3 - 2)), left=max(1, (_my_win.swidth-_win_wid)/2), title=wintitle)
    WindowList.add_window(_my_win.win)
    _my_win.win.bkgd(' ', curses.color_pair(5))
    _tabarr = list()
    _start = 3
    ...
    while True:
        ...
        _my_win.win.addstr(_tabarr[_idx][1], _tabarr[_idx][0], _tabarr[_idx][2], curses.A_REVERSE) # highlight new choice
        _my_win.win.addstr(_tabarr[_prev_index][1], _tabarr[_prev_index][0], _tabarr[_prev_index][2]) # unhighlight old choice
        _my_win.win.move(_tabarr[_idx][1], _tabarr[_idx][0]) # Move cursor to new choice
        _my_win.win.refresh()
        _prev_index = _idx
...
'''
Displays a short list of strings vertically that the user can choose from, tabbing to highlight and select.
e.g., choice = popupGetMultipleChoice('Choose one:', ['Yes', 'No', 'Elephant', 'Mouse', 'Green', 'Red'], 'Mouse')
'''
def popup_get_multiple_choice_vert(s, wintitle, choices, default, sw, log):
    ...
    _my_win.create(_win_ht, _win_wid, top=max(1, (_my_win.sheight/3 - _win_ht/2)) , left=max(1, (_my_win.swidth-_win_wid)/2), title=wintitle)
    WindowList.add_window(_my_win.win)
    _my_win.win.bkgd(' ', curses.color_pair(5))
    _tab_arr = list()
    _prev_index = 0
    ...
    while True:
        ...
        _my_win.win.addstr(_tab_arr[_idx][1], _tab_arr[_idx][0], _tab_arr[_idx][2], curses.A_REVERSE) # highlight new choice
        _my_win.win.addstr(_tab_arr[_prev_index][1], _tab_arr[_prev_index][0], _tab_arr[_prev_index][2]) # unhighlight old choice
        _my_win.win.move(_tab_arr[_idx][1], _tab_arr[_idx][0]) # Move cursor to new choice
        _my_win.win.refresh()
        _prev_index = _idx
...
def popup_get_text(s, wintitle, sw, log):
    ...
    _my_win.create(5, _my_wid, top=max(1, _my_win.sheight/3 - 2), left=max(1, (_my_win.swidth-_my_wid)/2), title=wintitle)
    WindowList.add_window(_my_win.win)
